...
// START FIX inifinte recursion
#define RECURSION_LIMIT 200

int Catalog::countPageTreeSafe(Object *pagesObj, int recursion_level) {
  ...
  if (pagesObj->dictLookup("Kids", &kids)->isArray()) {
    ...
    for (i = 0; i < kids.arrayGetLength(); ++i) {
      kids.arrayGet(i, &kid);
      recursion_level++;
      n2 = countPageTreeSafe(&kid, recursion_level);
      if (n2 < INT_MAX - n) {
	n += n2;
      } else {
...
int Catalog::countPageTree(Object *pagesObj) {
  return this->countPageTreeSafe(pagesObj, 0);
}
// END FIX infinite recursion
...
// START FIX infinite recursion
#define RECURSION_LIMIT 200

void Catalog::readPageLabelTree2Safe(Object *node, int recursion_level) {
  ...
  if (node->dictLookup("Kids", &kids)->isArray()) {
    for (i = 0; i < kids.arrayGetLength(); ++i) {
      kids.arrayGet(i, &kid);
      recursion_level++;
      readPageLabelTree2Safe(&kid, recursion_level);
      kid.free();
    }
  }
  kids.free();
...
void Catalog::readPageLabelTree2(Object *node) {
  this->readPageLabelTree2Safe(node, 0);
}
// END FIX infinite recursion
