...
//			compose_image_filename(dev_picture_base, ++dev_picture_number, extension, pic_file);
            ...
            // START FIX too much memory
            # define MEM_LIMIT 10000000000
            size_t array_size = stride * height;
            if (array_size > MEM_LIMIT) {
                std::cerr << "Tried to allocate too much memory." << std::endl;
                exit(-1);
            }
            unsigned char *data = new unsigned char[array_size];
            // END FIX too much memory
